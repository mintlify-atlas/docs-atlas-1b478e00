---
title: Changelog
description: Complete version history and changes for IronOS firmware
---

This page tracks all versions and changes to the IronOS firmware. The current stable release is **v2.23**.

## v2.23

### High level changes

- Miniware I2C changed to bit-bang for improved compatibility with newer devices with STM32 clone ICs
...
- Fixes for USB-PD to implement device capabilities. This should fix unit reboot issues with some laptops
- Lots of translation & documentation updates
- Fixes for RTOS issues / updated FreeRTOS version
- Multiple fixes for MHP30 to help with clone STM32s
- Share missing settings over BLE for Pinecil V2
- Add code for `ws2812b` LED mod for Pinecil V2 (Must be hand compiled)
...
The [Sequre S60](https://sequremall.com/products/sequre-s60-nano-electric-soldering-iron-support-pd-qc-power-supply-compatible-with-c210-soldering-iron-tips-precision-electronic-mobile-phone-repair-tool-anti-static-soldering-pen) uses JBC tips, which makes it quite useful for the smaller tip types and extra options available.
...
Absolutely massive kudos goes to @VioletEternity for her work on the reverse engineering of this. If you at all are helped by IronOS running on this device more credit goes to her than to I. Also big thanks to @whitequark for organizing + supporting + magic.
...
2. Bootup Logo support is finalized and working.
...
For Pinecil V2 users blisp is currently the recommended CLI tool for updating the device. It is built for all main OSs automatically. This does not apply to V1 devices. If your iron came with a blue grip, its a V1 and update the same as always. If your device came with a green silicone grip its a V2 device.

Alternatively you can use Spagett1's PineFlash tool that should provide a GUI interface for PinecilV1 & PinecilV2.

For a small number of V2 Pinecil devices there appears to be an interference issue between the Bluetooth Low Energy and some devices. If this occurs to you, please let us know in the issue and rollback to 2.20 for now.
...
- Support for animated bootup logos
- Bootup logos moved to their own IronOS-Meta repo
...
- Fixed automatic orientation for newer TS80Ps with the SC7 accelerometer
...
- Hall effect sensor is not fitted to Pinecils by default, you have to fit this yourself if you want the feature
...
- Added a small tolerance to allow "overvoltage" on QC3 above unit specifications (at your own risk)
...
- Every language had translation updates
...
Also kudos to @Vinigas and @GoJian for helping with testing.
This is not a final version, but this is a working, usable version of firmware support.
...
- Adds support for two new accelerometers (SC7A20 and MSA301)
- Add warnings if accelerometer or USB-PD ICs are not detected
- Fixed cooling down blink to be sane speed
- Cleanup of threads and slightly faster power negotiation
- Updates to flashing scripts
- Documentation updates all over the place
- Updates to makefile
- Cleanup the folder name of the source code
- clang-format spec setup
...
- Repo rename occurred TS100 → IronOS
...
- Fixes issue where going from COOL to soldering can leave screen off
...
- Adds hold both buttons on IDLE to access the thermometer mode
